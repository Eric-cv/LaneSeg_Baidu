...
def main():
    ...
    path = r'H:\ZQ_file\project01\utils\data_list\train.csv'
    data = pd.read_csv(path, header=None, names=["image", "label"])
    images = data["image"].values[1:]

    for idx in range(len(images)):

        imgFileName = images[idx]
        ...
        # 保存结果图
        outputPath = 'test_example/testImages'
        if not os.path.exists(outputPath):
            os.makedirs(outputPath)
        imgOutputFilename = os.path.join(outputPath,"epoch%dRes_%05d.jpg" % (epoch, idx))
