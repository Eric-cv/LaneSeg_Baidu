...
def encode_labels(color_mask):
    # 输入：ori_mask，输出：encode_mask
    # 由于数据集中label各类所对应的标签不连续，因此要对label中的id标签转换为方便的train id(1-7,背景0，8类)
    # 定义与mask同样大的全0数组（全背景）
...
def decode_labels(labels):
    # 将推断得到的gray mask的train id转为gray id形式
    # 输入：mask 输出：decode_mask
...
def decode_color_labels(labels):
    # 将推断得到的mask的train id转为color id形式
    # 输入：mask 输出：decode_mask
...
def verify_labels(labels):
    # 验证labels,验证mask上的所有label，得到除了背景0以外的所有label字典pixels
    pixels = [0]
